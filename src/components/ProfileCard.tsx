

type ProfileCardProps = {
    profilePic?: string;
    name: string;
    linkedin: string;
    github: string;
    instagram: string
}

const ProfileCard = ({profilePic, name, linkedin, github, instagram}: ProfileCardProps) => {
    return(
        <>
            <div className="profile-card">
                <div className="image" >
                    <img src={profilePic} alt="profileImg" className="profile-image " />
                </div>

                <div className="info">
                    <span className="name">{name}</span>
                </div>

                <div className="media-buttons">
                    <a href={linkedin} className="link linkedin" target='_blank'>
                    <img src="data:image/png;base64,"/>
                    </a>

                    <a href={github} className="link github" target='_blank'>
                    <img src="data:image/png;base64,"/>
                    </a>

                    <a href={instagram} className="link instagram" target='_blank'>
                    <img src="data:image/png;base64,"/>
                    </a>
                </div>

                

            </div>


        </>
    )
};

export default ProfileCard;